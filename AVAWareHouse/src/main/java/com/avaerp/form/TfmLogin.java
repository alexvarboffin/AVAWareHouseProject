...
//        Log.d(TAG, "@@@@12 : " + prefs1.getString("user_name", ""));
    ...
    protected boolean checkRequires() {
        ...
        if (result && (staffId.length() > 8 || staffId.isEmpty() || staffId.equals("0"))) {
            result = false;
    ...
    public void onClick(View aView) {
        super.onClick(aView);
        int id = aView.getId();
        if (id == R.id.TfmLogin_cbParams) {
            getView(R.id.TfmLogin_pnParams).setVisibility(getCheckBox(aView).isChecked() ? View.VISIBLE : View.GONE);
        } else if (id == R.id.TfmLogin_btConnect) {
            if (checkRequires()) {
                saveControls();
                login();
            } else if (!getCheckBox(R.id.TfmLogin_cbParams).isChecked()) {
                if ((getTextViewText(R.id.TfmLogin_edHostName).isEmpty()) ||
                        (getTextViewText(R.id.TfmLogin_edPort).isEmpty()) ||
                        (getTextViewText(R.id.TfmLogin_edSid).isEmpty())) {
    ...
    public void onSqlExecuted(int aSqlId, TParams aParams, TSqlExecutor.TExecuteStatus aStatus) {
        if (aStatus == TSqlExecutor.TExecuteStatus.SUCCESSFUL) {
            if (aSqlId == R.string.sql_check_user_password) {
    ...
    public void afterSqlQuery(int aSqlId, TParams aParams, ResultSet resultSet) {
        if (aSqlId == R.string.sql_get_version_no) {
            try {
        ...
        } else if (aSqlId == R.string.sql_get_version) {
            try {
                if (resultSet.next()) {
                    ...
                    Log.d(TAG, "afterSqlQuery: " + uri + " ### " + file.exists() + " ### " + file.length());
                }
            } catch (SQLException e) {
